  ...
  if(!empty($img))
  {
    ...
    //   print_r($img_desc);
      ...
      foreach($img_desc as $val)
      {
          if(strcmp(trim($val["name"]) , "")!=0){
            ...
            move_uploaded_file($val['tmp_name'],'../extradetails/'.$result);
          }
      }
    ...
    if($initializedExtraPhotosCount == $finalExtraPhotosCount){
      ...
      echo "Upload Successful";
        ...
        $stmt = $conn->prepare("INSERT INTO productsforsell(productpicture, productname, productbanglaname, productmetadata, productshortdetails, productcategories, minprice, maxprice, offeredprice, besafeless, sellername, extraimages, uploadedby, availability, availablein) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
